<script lang="ts">
	...
	let categories: any[] = [];
	let dummyCat: any[] = [
		{ name: 'Handbags', value: 'handbags', image: Handbag },
		{ name: 'Clothings', value: 'clothings', image: ClothesHanger },
		{ name: 'Jackets', value: 'jackets', image: Hoodie },
		{ name: 'Watches', value: 'watches', image: Wristwatch },
		{ name: 'Dresses', value: 'dresses', image: WeddingDress },
		{ name: 'Shoes', value: 'shoes', image: Shoes },
		{ name: 'Jeans', value: 'jeans', image: Trousers },
		{ name: 'Shirts', value: 'shirts', image: Tshirt }
	];
	...
	async function getProductCategories() {
		...
		//       "/customer/category?is_top_category=true"
		//     );
		//     this.categories = Array.isArray(data?.results)
		//       ? data.results
		//       : this.dummyCat;
		//   } catch (error) {
...
<div class="xl:max-w-7xl lg:max-w-4xl mx-auto md:py-20 pb-6">
	...
	<div class="flex items-center justify-center mx-auto self-center">
		{#each categories as category}
			...
			>
				<div
					class="md:h-28 md:w-28 bg-white p-2 rounded-full flex items-center justify-center overflow-hidden"
				>
					<!-- <img
              ...
              /> -->
				...
				<h1 class="text-sm font-medium text-head text-center">
					{category.name}
				</h1>
			</button>
		{/each}
